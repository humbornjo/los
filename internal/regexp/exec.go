...
func (i *inputs) newBytes(b []byte) input {
	i.bytes.str = bytes.NewBuffer(b)
	return &i.bytes
}

func (i *inputs) extend(s string) input {
	i.bytes.str.WriteString(s)
	return &i.bytes
}
...
func (re *Regexp) doExecute(
	r io.RuneReader, b []byte, s string, pos int, ncap int, dstCap []int,
) []int {
	if dstCap == nil {
